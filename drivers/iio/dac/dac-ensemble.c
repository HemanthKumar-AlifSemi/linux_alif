...
static int ensemble_dac_probe(struct platform_device *pdev)
{
	...
	analog_base = ioremap(ANALOG_BASE, SZ_64);
	reg = readl(analog_base + VBAT_ANA_REG2);
	reg |= (ANA_PERIPH_BG_ENA | ANA_PERIPH_LDO_EN);
	writel(reg, analog_base + VBAT_ANA_REG2);
	iounmap(analog_base);
