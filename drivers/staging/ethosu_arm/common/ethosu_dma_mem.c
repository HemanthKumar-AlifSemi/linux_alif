					    ...
					    size_t size)
{
	...
	kref_init(&dma_mem->kref);

	return dma_mem;
}

static void ethosu_dma_mem_destroy(struct kref *kref)
{
	struct ethosu_dma_mem *dma_mem =
		container_of(kref, struct ethosu_dma_mem, kref);
	struct device *dev = dma_mem->dev;

	dma_free_coherent(dev, dma_mem->size, dma_mem->cpu_addr,
			  dma_mem->dma_addr);
	memset(dma_mem, 0, sizeof(*dma_mem));
	devm_kfree(dev, dma_mem);
}

void ethosu_dma_mem_get(struct ethosu_dma_mem *dma_mem)
{
	kref_get(&dma_mem->kref);
}

void ethosu_dma_mem_put(struct ethosu_dma_mem *dma_mem)
{
	kref_put(&dma_mem->kref, ethosu_dma_mem_destroy);
}
