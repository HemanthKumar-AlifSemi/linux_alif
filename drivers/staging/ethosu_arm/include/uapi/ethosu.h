...
#define ETHOSU_IOCTL_BUFFERS_CREATE     ETHOSU_IOWR(0x11, \
						    struct ethosu_uapi_buffers_create)
#define ETHOSU_IOCTL_BUFFER_CLEAR       ETHOSU_IO(0x12)
...
#define ETHOSU_KERNEL_DRIVER_VERSION_MAJOR 6
#define ETHOSU_KERNEL_DRIVER_VERSION_MINOR 1
...
/**
 * struct ethosu_uapi_buffers_create - Create buffers request
 * @num:	[in] Number of buffers to create
 * @size:	[in] Size of individual buffer
 * @fds:	[out] File descriptors for the requested buffers
 *
 * The buffers returned can only be accessed via read/write calls. Memory
 * mapping them with mmap is not supported.
 */
struct ethosu_uapi_buffers_create {
	__u32           num;
	__kernel_size_t sizes[ETHOSU_FD_MAX];
	int             fds[ETHOSU_FD_MAX];
};
 ...
 * @network_mask:              Index mask for available networks
 */
struct ethosu_uapi_device_capabilities {
 ...
 * @intermediate_fd:    Intermediate buffer file descriptor
 * @constant_fd:        Constant data buffer file descriptor
 * @pmu_config:         PMU configuration
 *
 * The @intermediate_fd and @constant_fd variables are ignored in the
 * ETHOSU_UAPI_DEVICE_SUBSYSTEM setup
 */
struct ethosu_uapi_inference_create {
	__u32                         ifm_count;
