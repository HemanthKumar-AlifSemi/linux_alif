...
int ethosu_rpmsg_network_info_request(struct ethosu_network *net,
				      struct ethosu_uapi_network_info *uapi)
{
	...
	ret = ethosu_rpmsg_mailbox_register(rp_net->mailbox, &info->msg);
	if (ret < 0) {
		dev_err(dev,
	...
	ret = ethosu_rpmsg_network_info_send(info, rp_net->mailbox);
	if (ret)
		goto deregister;
...
deregister:
	ethosu_rpmsg_mailbox_deregister(rp_net->mailbox, &info->msg);
