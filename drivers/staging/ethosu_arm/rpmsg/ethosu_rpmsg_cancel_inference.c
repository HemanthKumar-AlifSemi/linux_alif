...
int ethosu_rpmsg_cancel_inference_request(struct ethosu_inference *inf,
					  struct ethosu_uapi_cancel_inference_status *uapi)
{
	...
	ethosu_inference_get(&rp_inf->inf);
	/* mark inference ABORTING to avoid resending the inference message */
	rp_inf->inf.status = ETHOSU_UAPI_STATUS_ABORTING;

	cancellation->dev = dev;
	cancellation->inf = rp_inf;
	...
	if (rp_inf->inf.status != ETHOSU_UAPI_STATUS_ABORTED)
		rp_inf->inf.status = ETHOSU_UAPI_STATUS_ABORTED;
...
kfree:
	...
	ethosu_inference_put(&cancellation->inf->inf);
