...
static void ethosu_rpmsg_buffer_set_from_buffer(
	const struct ethosu_buffer *buffer,
	struct ethosu_rpmsg_buffer *cbuf)
{
	cbuf->ptr = (uint32_t)ethosu_buffer_to_dma_addr(buffer);
	cbuf->size = (uint32_t)buffer->size;
}

static void ethosu_rpmsg_buffer_set_from_network(
	const struct ethosu_network *network,
	struct ethosu_rpmsg_buffer *cbuf)
{
	cbuf->ptr = (uint32_t)network->dma_mem->dma_addr;
	cbuf->size = (uint32_t)network->dma_mem->size;
}
				   ...
				   uint8_t pmu_cycle_counter_enable)
{
	...
	for (i = 0; i < ifm_count; i++)
		ethosu_rpmsg_buffer_set_from_buffer(ifm[i], &inf_req->ifm[i]);

	for (i = 0; i < ofm_count; i++)
		ethosu_rpmsg_buffer_set_from_buffer(ofm[i], &inf_req->ofm[i]);

	for (i = 0; i < ETHOSU_RPMSG_PMU_MAX; i++)
		inf_req->pmu_event_config[i] = pmu_event_config[i];

	if (network->net.dma_mem != NULL) {
		inf_req->network.type = ETHOSU_RPMSG_NETWORK_BUFFER;
		ethosu_rpmsg_buffer_set_from_network(&network->net,
						     &inf_req->network.buffer);
	} else {
		inf_req->network.type = ETHOSU_RPMSG_NETWORK_INDEX;
		inf_req->network.index = network->net.index;
	}
					      ...
					      struct ethosu_rpmsg_network *network)
{
	...
	if (network->net.dma_mem != NULL) {
		...
		info_req->network.index = network->net.index;
	}
